...
## [2024-12-22] - Исправление отображения Google Maps в модальном окне

### 🐛 Исправлено
- **Критический баг:** Google Maps не отображались в модальном окне `MapPreviewModal`
  - **Проблема:** Content Security Policy (CSP) блокировал iframe от `maps.google.com`
  - **Симптомы:** Модальное окно открывалось, но карта была пустой
  - **Root Cause:** `frame-src` в `next.config.ts` не включал Google Maps домены
  - **Решение:** Добавлены `https://maps.google.com` и `https://www.google.com` в `frame-src` directive

### ✨ Изменения
- **next.config.ts:** Обновлена CSP конфигурация
  ```typescript
  "frame-src https://telegram.org https://oauth.telegram.org https://maps.google.com https://www.google.com"
  ```

### 📚 Документация
- Создан `docs/FIX_GOOGLE_MAPS_CSP.md` с полным анализом проблемы
- Описаны шаги диагностики CSP ошибок
- Best practices для настройки CSP
