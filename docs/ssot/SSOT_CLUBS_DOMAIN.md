...
Flow:
Owner creates invite targeting a specific user.
Invitee accepts.
Membership becomes member (default) unless owner explicitly assigns admin after acceptance.
Rules:
...
Flow:
Owner generates invite link (token).
User opens link while authenticated.
System creates/uses join request or direct acceptance policy (see below).
...
Flow:
User requests to join a club (only if club is public OR has a known link).
Owner approves/rejects.
On approve: membership created as member.
Rules:
...
target must be active member/admin (not pending)
after transfer:
...
(See SSOT_CLUBS_EVENTS_ACCESS, SSOT_BILLING.)
...
This SSOT defines error codes as outcomes; HTTP mapping is owned by SSOT_API/ARCHITECTURE.
Canonical codes:
UNAUTHORIZED
FORBIDDEN
VALIDATION_ERROR
...
Foundation
entities, invariants, visibility, membership entry methods, audit, idempotency
Club Page
profile read/edit rules; public/private exposure
Members
invites, join requests, leave/kick, role changes, ownership transfer
Events
by SSOT_CLUBS_EVENTS_ACCESS (already LOCKED)
Trust
badges as scoped permissions (no RBAC inflation)
Content
content types, visibility flags, moderation slots
Monetization
owner-only billing authority + subscription hooks by reference
Network
discovery + relationships without implicit membership
