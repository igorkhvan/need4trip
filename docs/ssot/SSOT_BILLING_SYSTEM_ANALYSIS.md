...
- 🔗 Added cross-reference to SSOT_ARCHITECTURE.md §8.3 (Auth Context Types)
...
1. **Billing — user-initiated domain** — все billing операции инициируются пользователем. Аутентификация обеспечивается платформенной инфраструктурой и не является ответственностью billing домена. (См. SSOT_ARCHITECTURE.md §8.3)
2. **Frontend не решает лимиты и доступ** — фронт только показывает UI и реагирует на ошибки backend
3. **Backend — единственный источник истины** — по доступу, лимитам, grace и paywall
4. **Цены и лимиты — в БД** — seed + API `/api/plans`
5. **Поведение при неоплате — в БД** — grace period, доступные действия в `pending`/`grace`/`expired`
6. **Транзакции не участвуют в проверках доступа** — `billing_transactions` — только аудит/история
